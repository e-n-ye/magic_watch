...
- 先观察当前代码和文档，再改动。
- 只改本次目标需要的文件。
- 代码事实一旦成立，默认在同一轮同步最小必要文档；不要把代码收口和状态文档收口默认拆成两轮。
- 实现类 prompt 默认同时允许：本轮代码文件、对应状态文档；只有在判定规则发生变化时，才额外允许修改 `.agents/skills/doc-memory-workflow/SKILL.md`。
- 如果本轮 prompt 明确禁止改文档，则不要越权补文档；应在收尾时如实报告 `Doc Impact`，并明确下一轮需要文档同步。
- 较大修改前说明影响面。
- 修改后运行可行的验证命令。
- 不因清爽而做无关重构。
...
4. 小步实现。
5. 验证结果。
6. 在同一逻辑闭环内更新最小必要文档或决策记录；只有本轮明确禁止改文档时，才拆到下一轮。
