...
F411 工程只在平台端口层接入同一套生成 C 和 `watch_core`，并负责真实硬件显示链路、输入链路和性能验证。
...
- 在 PC 闭环完成后接入同一套 XML 生成 UI 和 `watch_core`。

不职责：

- 不承载业务核心的唯一实现。
- 不在平台 driver 中解释页面业务。
- 不在 V0.3 兼容性探针前跳过刷新诊断和资源评估直接迁移完整 UI。
...
F411 验收在 PC 闭环之后进行。当前先完成 V0.0 DMA 指标收口，再执行 V0.1 刷新率诊断、V0.2 减少无效刷新和 V0.3 XML 兼容性探针；只有探针结论允许继续时，才进入 V0.4 真机最小闭环。
...
5. `V0.4`：仅在 V0.3 结论允许继续后，再规划 F411 XML 真机最小闭环。
