...
这一阶段不要让 AI 一口气生成完整 core，也不要把已有模块误判成“从零待造”。真实学习路径应拆成当前这组小闭环：

- `V0.5-P0`：先给现有 `watch_core` 加纯 PC 合同测试，并审计 F411 当前主链所有权。
- `V0.5-P1`：把页面状态、瞬时页面动作和 Adapter 消费规则拆开，停止让平台自己猜首页。
- `V0.5-P2`：把 pending drain 的“处理完成”语义收紧到队列空，并明确第三 Adapter 接入门。
- `V0.5-P3`：建立最小 Power 决策合同，确认 Core 决策和平台执行分层。
- `V0.5-P4`：把学习沉淀、路线修正和 `V0.6` 交界写清，避免后续窗口回到过时路线。

旧 `V0.5-A~G` 的学习目标没有消失，但它们现在要通过“审计真实代码、收口共享合同、判断边界所有权”的方式掌握，而不是机械新造一套同名模块。
