...
只需要新增menu页面作为一个中转页面进行判断
最后都可以路由到同一个heart或是steps页面

- 菜单选中项离开菜单后是否需要保留？
可以保留也可以不保留

- `back` 的规则应该按命令集中写，还是分散到各页面处理中？
从不同入口进入app页面，back返回的语义不同，集中写可能缺少上下文去判断back位置，分散写可能增加后期维护压力
我更加倾向于使用一个全局变量去记忆上一页面，集中写
...
语义转化，应该在实际进入同一页面有多种路口时可以考虑，进行用户意图转化

- 哪种实际重复出现后，才考虑 Renderer 边界？
多种页面有同一类似的渲染风格可以考虑，渲染边界
...
- 2026-06-14：用户已填写事前预测。预测中“仅有 current_page 足够”与“需要全局上一页面”存在冲突，已保留为本轮验证问题；当前建立可编译练习骨架，不替用户选择返回状态模型。
- 2026-06-14：Stage 02 Console target 配置与构建通过；短运行确认 Home/Heart/Steps 旧行为可达，`menu` 仍进入未实现路径。菜单选中项、返回来源和新增导航规则均保留给用户完成。
- 2026-06-14：用户完成菜单导航并发现可能需要多次 Back。审查后确认当前最大深度仍可由详情返回目标覆盖；Agent 完成机械收口，修复 Home 提前返回、错误分类和枚举算术映射。完整命令矩阵通过，Stage 02 冻结。

### Stop policy

- 实现骨架构建后停止，等待用户填写导航表并完成四个 `LEARN` 区域。
- 用户实现验收和复盘完成后冻结，不自动实现下一 Stage。
