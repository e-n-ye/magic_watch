...
- `HeartRateAppPage` 的 entry measurement timer 已迁入 `LvglTimerGuard`；同页的 crown release timer、wear prompt 显示边界与 stage 切换仍待后续单独审计。
- 当前非测量域页面包括：
  - `HeartRateInfoPage`
  - `HeartRateAppPage`
...
- 当前更集中的风险仍在 `HeartRatePages.cpp` 内部，但已经从“measurement timer + stage + wear prompt 全部混在一起”收窄到“crown release timer、stage 切换与 wear prompt 生命周期边界”。
