...
)
{
  /* USER CODE BEGIN READ */
    ...
    /* Convert sector to byte address (512 bytes per sector) */
    flash_addr = sector * 512;

    /* Read data from W25Q128 */
    if (bsp_w25q128_read(flash_addr, buff, count * 512) == 0)
    {
        return RES_OK;
    }
...
)
{
  /* USER CODE BEGIN WRITE */
    ...
    /* Write sector by sector with erase */
    while (write_len > 0)
    {
        ...
        for (uint32_t i = 0; i < sec_remain; i++)
            sector_buffer[sec_offset + i] = buff[offset + i];
        ...
        {
            if (bsp_w25q128_write_page(sec_addr + i,
                                         sector_buffer + i,
                                         W25Q128_PAGE_SIZE) != 0)
                return RES_ERROR;
        }
...
)
{
  /* USER CODE BEGIN IOCTL */
    ...
    case GET_SECTOR_COUNT:	/* Get number of sectors on the disk (DWORD) */
        *(DWORD*)buff = W25Q128_CAPACITY / 512;  /* 16MB / 512 = 32768 sectors */
        res = RES_OK;
        break;
    ...
    case GET_BLOCK_SIZE:	/* Get erase block size in unit of sector (DWORD) */
        *(DWORD*)buff = W25Q128_SECTOR_SIZE / 512;  /* 4096 / 512 = 8 sectors */
        res = RES_OK;
        break;
