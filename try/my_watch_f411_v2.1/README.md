...
- LVGL 已按最小 label bring-up 目标收窄配置并登记到 MDK 工程；当前已有项目自有 display flush port，flush 复用 `watch_lcd_draw_rgb565()`；`lv_timer_handler()` 由 FreeRTOS defaultTask 通过 `watch_bringup_task()` 单任务驱动，尚未接 encoder indev 或 LVGL screen。
